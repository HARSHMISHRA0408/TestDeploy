...
const AssignTest = ({ user }) => {
  ...
  return (
    <Layout user={user}>
      <div className="w-4/5 mx-auto mt-10 p-4 bg-white shadow-lg rounded-lg">
        <div className="flex justify-between">
          <h2 className="text-xl font-semibold text-gray-700 mb-4">Assign Test to Users</h2>
          <Link href="/manager/ManageTest" >✖️</Link>
        </div>
        ...
        {!loading && !error && (
          <form onSubmit={handleSubmit}>
            ...
            <div className="max-h-40 overflow-y-auto border p-2 rounded">
              {users.length > 0 ? (
                users.map((user) => (
                  <label key={user._id} className="cursor-pointer flex">
                    <input
                      type="checkbox"
